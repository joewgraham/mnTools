...
for mnnum = 1 : numbermns
    ...
    for i = 2:length(mnpop.mn(mnnum).piece)
        ...
        if mnpop.mn(mnnum).piece(i).length == 0
            ...
            mnpop.mn(mnnum).piece(i).taperrateback           = 0;
			mnpop.mn(mnnum).piece(i).taperrateforward           = 0;
        else
            mnpop.mn(mnnum).piece(i).cylindricaldiameter = 2*((mnpop.mn(mnnum).piece(i).volume/(pi*mnpop.mn(mnnum).piece(i).length))^0.5);
            mnpop.mn(mnnum).piece(i).taperrateback           = (mnpop.mn(mnnum).piece(i).proximaldiameter - mnpop.mn(mnnum).piece(i).distaldiameter) / mnpop.mn(mnnum).piece(i).length;
			mnpop.mn(mnnum).piece(i).taperrateforward           = (mnpop.mn(mnnum).piece(i).proximaldiameter - mnpop.mn(mnnum).piece(i).distaldiameter) / mnpop.mn(mnnum).piece(i).length;
        end
        mnpop.mn(mnnum).piece(i).csa                 = pi*((mnpop.mn(mnnum).piece(i).cylindricaldiameter/2)^2);

        if isnan(mnpop.mn(mnnum).piece(i).taperrateback) || isnan(mnpop.mn(mnnum).piece(i).taperrateforward)
            disp('Error in ''importswc''.');
            disp('Taper rate set to zero from NaN.');
            mnpop.mn(mnnum).piece(i).taperrateback = 0;
			mnpop.mn(mnnum).piece(i).taperrateforward = 0;
        end
        if isinf(mnpop.mn(mnnum).piece(i).taperrateback) || isinf(mnpop.mn(mnnum).piece(i).taperrateforward)
            disp('Error in ''importswc''.');
    ...
    for j = 2:length(mnpop.mn(mnnum).piece)
        ...
        if strcmp(option.mnimport.pieceshape,'cylindrical') == 1
			if mnpop.mn(mnnum).piece(j).taperrateback == 0
                if strcmp(mnpop.mn(mnnum).piece(j).distaltype,'bifurcation')==1 || strcmp(mnpop.mn(mnnum).piece(j).distaltype,'termination')==1
                	mnpop.mn(mnnum).piece(j).taperrateback = 0;
					%mnpop.mn(mnnum).piece(j).taperratevolume = 0;
                else
                    if mnpop.mn(mnnum).piece(j).length == 0
                        mnpop.mn(mnnum).piece(j).taperrateback = 0;
                    else
                        mnpop.mn(mnnum).piece(j).taperrateback = (mnpop.mn(mnnum).piece(j).distaldiameter - mnpop.mn(mnnum).piece(j+1).proximaldiameter) / mnpop.mn(mnnum).piece(j).length;
                ...
                if isnan(mnpop.mn(mnnum).piece(i).taperrateback)
			...
			if mnpop.mn(mnnum).piece(j).taperrateforward == 0
                if strcmp(mnpop.mn(mnnum).piece(j).proximaltype,'primary    ')==1 || strcmp(mnpop.mn(mnnum).piece(j).proximaltype,'daughter1  ')==1 || strcmp(mnpop.mn(mnnum).piece(j).proximaltype,'daughter2  ')==1
                	mnpop.mn(mnnum).piece(j).taperrateforward = 0;
				else
                    if mnpop.mn(mnnum).piece(j).length == 0
                        mnpop.mn(mnnum).piece(j).taperrateforward = 0;
                    else
                        mnpop.mn(mnnum).piece(j).taperrateforward = -(mnpop.mn(mnnum).piece(j).proximaldiameter - mnpop.mn(mnnum).piece(mnpop.mn(mnnum).piece(j).parent).distaldiameter) / mnpop.mn(mnnum).piece(j).length;
                    end
                end
                if isnan(mnpop.mn(mnnum).piece(i).taperrateforward)
                    disp('Error (NaN) in ''importswc''.');
                    keyboard;
                end
                if isnan(mnpop.mn(mnnum).piece(i).taperrateforward)
    ...
    for piecenumber = 2:length(mnpop.mn(mnnum).piece)
		mnpop.mn(mnnum).piece(piecenumber).taperrate = (mnpop.mn(mnnum).piece(piecenumber).taperrateback + mnpop.mn(mnnum).piece(piecenumber).taperrateforward) / 2;
	end
