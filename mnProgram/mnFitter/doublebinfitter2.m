function parameterfits = parameterfitter6(analysis,parameterfits)

disp('   Fitting doublebin bif/term taper rate.');
options;
                    ...
                    for k = 1:length(fittypes)
                        ...
                        for measurecolumn = 1:size(measuredata,2)
                            ...
                            if (sum(~isnan(y))-length(y(y==0))) > 2
                                ...
                                eval(['parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.parameter(' num2str(fitcount) ',:) = parameter;']);
                                eval(['parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.rmse(' num2str(fitcount) ',:) = rmse(1);']);
                                eval(['parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.weight(' num2str(fitcount) ',:) = ' num2str(sum(weight)) ';']);
                                eval(['parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.' secondbin '(' num2str(fitcount) ',:) = ' num2str(x2(1,1)) ';']);
                        ...
                        eval(['meanrmse = mean([parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.rmse]);']);
                        eval(['totalrmse = sum([parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.rmse]);']);
                        eval(['parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.meanrmse = meanrmse;']);
                        eval(['parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.totalrmse = totalrmse;']);
                        ...
                        if strcmp(firstbinfigures,'y') == 1
                            ...
                            filename = sprintf('%s_ParameterFits_%03d_DoubleBin_%s_%s_%s_vs_%s_%s_%s',analysis.inputfilename,figurecount,branch,measure,statistic,firstbin,fit,fittype);
                            cd(mainfolder);
                        end
                        if strcmp(firstbinfigures,'y') == 1 || strcmp(secondbinfigures,'y') == 1
                        ...
                        if strcmp(firstbinfigures,'y') == 1
                            ...
                            if strcmp(fit,'exponential') == 1
                                set(gca,'ZScale','log');
                                filename = sprintf('%s_ParameterFits_%03d_DoubleBin_%s_%s_%s_vs_%s_%s_%s_logplot',analysis.inputfilename,figurecount,branch,measure,statistic,firstbin,fit,fittype);
                                print('-djpeg',filename);
                            elseif strcmp(fit,'power') == 1
                                set(gca,'ZScale','log');
                                set(gca,'XScale','log');
                                filename = sprintf('%s_ParameterFits_%03d_DoubleBin_%s_%s_%s_vs_%s_%s_%s_loglogplot',analysis.inputfilename,figurecount,branch,measure,statistic,firstbin,fit,fittype);
                                print('-djpeg',filename);
                            end
                        end
                        ...
                        if strcmp(secondbinfits,'y') == 1
                            ...
                            for m = 1:length(parameter)
                                ...
                                eval(['x = [parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.' secondbin '];']);
                                eval(['y = [parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.parameter(:,' num2str(m) ')];']);
                                eval(['weight = [parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.weight];']);
                                weight = weight(isfinite(y));
                                x      = x(isfinite(y));
                                y      = y(isfinite(y));
                                ...
                                for n = 1:length(parameterfits)
                                    ...
                                    if strcmp(secondbinfigures,'y') == 1
                                        ...
                                        title(char(' ',[branchlabel ' ' measurelabel ' ' statisticlabel ' vs ' firstbintitle ' -- ' fitlabel ' ' fittypelabel],[parametertitle ' vs ' secondbintitle ' -- ' parameterfitlabel ' Fits'],' '),'FontSize',12,'FontWeight','b');
                                        set(gca,'FontSize',6);
                                        hold on;
                                    end
                                    ...
                                    for o = 1:length(parameterfittypes)
                                        ...
                                        eval(['parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.parameter(' num2str(m) ').' parameterfit '.' parameterfittype '.parameter = parameter;']);
                                        eval(['parameterfits.' branch measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.parameter(' num2str(m) ').' parameterfit '.' parameterfittype '.rmse = rmse;']);
                                    ...
                                    if strcmp(secondbinfigures,'y') == 1
                                        ...
                                        filename = sprintf('%s_%03d_DB_%s_%s_%s_vs_%s_%s_%s_%02d_Prmtr%d_%s',analysis.inputfilename,figurecount,branch,measure,statistic,firstbin,fit,fittype,parameterfitcount,m,parameterfit);
                                        print('-djpeg',filename);
                                        if strcmp(parameterfit,'exponential') == 1
                                            set(gca,'YScale','log');
                                            filename = sprintf('%s_%03d_DB_%s_%s_%s_vs_%s_%s_%s_%02d_Prmtr%d_%s_logplot',analysis.inputfilename,figurecount,branch,measure,statistic,firstbin,fit,fittype,parameterfitcount,m,parameterfit);
                                            print('-djpeg',filename);
                                        elseif strcmp(parameterfit,'power') == 1
                                            set(gca,'YScale','log');
                                            set(gca,'XScale','log');
                                            filename = sprintf('%s_%03d_DB_%s_%s_%s_vs_%s_%s_%s_%02d_Prmtr%d_%s_loglogplot',analysis.inputfilename,figurecount,branch,measure,statistic,firstbin,fit,fittype,parameterfitcount,m,parameterfit);
                                            print('-djpeg',filename);
                                        end
                                    end
                                end
