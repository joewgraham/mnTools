function parameterfits = singlebinbiftermtaperratefitterbypiece(analysis,parameterfits,mnfitteroption)
            ...
            for g = 1:length(measures)
                ...
                for h = 1:length(versuses)
                    ...
                    if strcmp(versus,'branchorder')
                        eval(['x = [typeanalysis.' branch '.piece.' versus ']'';']);
                    else
                        eval(['x = [typeanalysis.' branch '.piece.proximal' versus ']'';']);
                    end
                    eval(['y = [typeanalysis.' branch '.piece.taperrate]'';']);
                    eval(['weight = [typeanalysis.' branch '.piece.' weightvar ']'';']);
                    ...
                    title(char(' ',[piecetitles{piecetype} ' ' branchlabel ' ' measurelabel ' vs ' versustitle],' '),'FontSize',12,'FontWeight','b');
                    set(gca,'FontSize',6);
                    filename = sprintf('%s_1_%s_SingleBin_%s_%s_vs_%s',analysis.inputfilename,piecetitles{piecetype},branch,measure,versus);
                    print('-djpeg',filename);
                    set(gca,'XScale','log');
                    filename = sprintf('%s_1_%s_SingleBin_%s_%s_vs_%s_logx',analysis.inputfilename,piecetitles{piecetype},branch,measure,versus);
                    print('-djpeg',filename);
                    %                     set(gca,'YScale','log');
                    %                     filename = sprintf('%s_%s_1_SingleBin_%s_%s_vs_%s_logx_logy',analysis.inputfilename,piecetitles{piecetype},branch,measure,versus);
                    %                     print('-djpeg',filename);
                    ...
                    if strcmp(currentoption.singlebins,'y') == 1
                        ...
                        for j = 1:length(fits)
                            ...
                            xlabel(versuslabel,'FontSize',8,'FontWeight','b');
                            ylabel(measurelabel,'FontSize',8,'FontWeight','b');
                            title(char(' ',[piecetitles{piecetype} ' ' branchlabel ' ' measurelabel ' vs ' versustitle ' -- ' fitlabel],' '),'FontSize',12,'FontWeight','b');
                            set(gca,'FontSize',6);
                            hold on;
                            ...
                            try
                                ...
                                for k = 1:length(fittypes)
                                    ...
                                    eval(['parameterfits.' branch  measure '.' versus '.' fit '.' fittype '.parameter = parameter;']);
                                    eval(['parameterfits.' branch  measure '.' versus '.' fit '.' fittype '.rmse = rmse;']);
                            ...
                            %                                 cd('singlebinfits');
                            %                             end
                            filename = sprintf('%s_%s_%03d_%s_%s_vs_%s_%s',analysis.inputfilename,piecetitles{piecetype},figurecount,branch,measure,versus,fit);
                            print('-djpeg',filename);
                            set(gca,'XScale','log');
                            filename = sprintf('%s_%s_%03d_%s_%s_vs_%s_%s_logx',analysis.inputfilename,piecetitles{piecetype},figurecount,branch,measure,versus,fit);
                            print('-djpeg',filename);
%                             set(gca,'YScale','log');
%                             filename = sprintf('%s_%s_%03d_%s_%s_vs_%s_%s_logx_logy',analysis.inputfilename,piecetitles{piecetype},figurecount,branch,measure,versus,fit);
%                             print('-djpeg',filename);
                            cd(mainfolder);
