function [parameter,rmse]=linearfitterrobustweighted(x,y,varargin)
...
if nargin > 2
    ...
    parameter = fminsearch(@(parameter) linearlorentzianweighted(x,y,weights,parameter) , parameter , optimset('Display','off'));
    rmse = [linearrmse(x,y,parameter) linearrmseweighted(x,y,weights,parameter)];
else
    parameter = fminsearch(@(parameter) linearlorentzian(x,y,parameter) , parameter , optimset('Display','off'));
