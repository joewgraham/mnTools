function [parameter,rmse]=exponentialoffsetfitterrobustweighted(x,y,varargin)
...
if nargin > 2
    parameter = fminsearch(@(parameter) exponentialoffsetrmseweighted(x,y,weights,parameter) , parameter , optimset('Display','off'));
    parameter = fminsearch(@(parameter) exponentialoffsetlorentzianweighted(x,y,weights,parameter) , parameter , optimset('Display','off'));
    rmse = [exponentialoffsetrmse(x,y,parameter) exponentialoffsetrmseweighted(x,y,weights,parameter)];
else
    parameter = fminsearch(@(parameter) exponentialoffsetlorentzian(x,y,parameter) , parameter , optimset('Display','off'));
