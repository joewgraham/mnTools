function [parameter,rmse]=poweroffsetfitterrobustweighted(x,y,varargin)
...
if nargin > 2
    % parameter = fminsearch(@(parameter) powerrmseweighted(x,y,weights,parameter) , parameter , optimset('Display','off'));
    % parameter = fminsearch(@(parameter) powerlorentzianweighted(x,y,weights,parameter) , parameter , optimset('Display','off'));
	parameter = fminsearch(@(parameter) poweroffsetrmseweighted(x,y,weights,parameter) , parameter);
    parameter = fminsearch(@(parameter) poweroffsetlorentzianweighted(x,y,weights,parameter) , parameter);
    rmse = [poweroffsetrmse(x,y,parameter) poweroffsetrmseweighted(x,y,weights,parameter)];
else
    % parameter = fminsearch(@(parameter) powerlorentzian(x,y,parameter) , parameter , optimset('Display','off'));
	    parameter = fminsearch(@(parameter) poweroffsetlorentzian(x,y,parameter) , parameter);
