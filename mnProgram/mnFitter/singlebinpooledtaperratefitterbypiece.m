function parameterfits = singlebinpooledtaperratefitterbypiece(analysis,parameterfits,mnfitteroption)

disp('     Single Bin Pooled Taper Rate by Piece.');
%mnfitteroptions;
    ...
    if isfield(analysis,piecetypes{piecetype}) && strcmp(fittypeoption,'y')
        ...
        for g = 1:length(measures)
            ...
            for h = 1:length(versuses)
                ...
                if strcmp(versus,'branchorder')
                    eval(['x = [[typeanalysis.bifurcation.piece.' versus '] [typeanalysis.termination.piece.' versus ']]'';']);  
                else
                    eval(['x = [[typeanalysis.bifurcation.piece.proximal' versus '] [typeanalysis.termination.piece.proximal' versus ']]'';']);  
                end
                y = [[typeanalysis.bifurcation.piece.taperrate] [typeanalysis.termination.piece.taperrate]]';
                eval(['weight = [[typeanalysis.bifurcation.piece.' weightvar '] [typeanalysis.termination.piece.' weightvar ']]'';']);
                ...
                if exist('Piecewise Mean','dir')==7
                    cd('Piecewise Mean');
                else
                    mkdir('Piecewise Mean');
                    cd('Piecewise Mean');
                end
                figure;
                ...
                ylabel(measurelabel,'FontSize',8,'FontWeight','b');
                title(char(' ',[piecetitles{piecetype} ' ' measurelabel ' vs ' versustitle],' '),'FontSize',12,'FontWeight','b');
                set(gca,'FontSize',6);
                filename = sprintf('%s_1_%s_%s_vs_%s',analysis.inputfilename,piecetitles{piecetype},measure,versus);
                print('-djpeg',filename);
                set(gca,'XScale','log');
                filename = sprintf('%s_1_%s_%s_vs_%s_logx',analysis.inputfilename,piecetitles{piecetype},measure,versus);
                print('-djpeg',filename);
%                 set(gca,'YScale','log');
%                 filename = sprintf('%s_1_%s_%s_vs_%s_logx_logy',analysis.inputfilename,piecetitles{piecetype},measure,versus);
%                 print('-djpeg',filename);
                ...
                if strcmp(currentoption.singlebins,'y') == 1
                    ...
                    for j = 1:length(fits)
                        ...
                        xlabel(versuslabel,'FontSize',8,'FontWeight','b');
                        ylabel(measurelabel,'FontSize',8,'FontWeight','b');
                        title(char(' ',[piecetitles{piecetype} ' ' measurelabel ' vs ' versustitle ' -- ' fitlabel],' '),'FontSize',12,'FontWeight','b');
                        set(gca,'FontSize',6);
                        hold on;
                        ...
                        try
                            ...
                            for k = 1:length(fittypes)
                                ...
                                eval(['parameterfits.' measure '.' versus '.' fit '.' fittype '.parameter = parameter;']);
                                eval(['parameterfits.' measure '.' versus '.' fit '.' fittype '.rmse = rmse;']);
                                
                                [fitx,fity] = rangefitter(fit,parameter,x);
                           ...
                           disp(' ');
                           disp('   Warning: not able to perform fit (singlebinpooledtaperratefitterbypiece).');
                           disp(' ');
                           continue
                        end
                        ...
                        %                                 cd('singlebinfits');
                        %                             end
                        filename = sprintf('%s_%s_%03d_%s_vs_%s_%s',analysis.inputfilename,piecetitles{piecetype},figurecount,measure,versus,fit);
                        print('-djpeg',filename);
                        set(gca,'XScale','log');
                        filename = sprintf('%s_%s_%03d_%s_vs_%s_%s_logx',analysis.inputfilename,piecetitles{piecetype},figurecount,measure,versus,fit);
                        print('-djpeg',filename);
%                         set(gca,'YScale','log');
%                         filename = sprintf('%s_%s_%03d_%s_vs_%s_%s_logx_logy',analysis.inputfilename,piecetitles{piecetype},figurecount,measure,versus,fit);
%                         print('-djpeg',filename);
                        cd(mainfolder);
