function parameterfits = doublebinpooledtaperratefitter(analysis,parameterfits,mnfitteroption)

disp('     Double Bin Pooled Taper Rate.');
%mnfitteroptions;
...
for g = 1:length(measures)
    ...
    for h = 1:length(firstbins)
        ...
        for i = 1:length(statistics)
            ...
            title(char(' ',[measurelabel ' ' statisticlabel  ' vs ' firstbintitle ' & ' secondbintitle],' '),'FontSize',12,'FontWeight','b');
            set(gca,'FontSize',6);
            filename = sprintf('%s_2_DoubleBin_%s_%s_vs_%s_&_%s',analysis.inputfilename,measure,statistic,firstbin,secondbin);
            cd(mainfolder);
            ...
            set(gca,'ZScale','log');
            filename = sprintf('%s_2_DoubleBin_%s_%s_vs_%s_&_%s_logz',analysis.inputfilename,measure,statistic,firstbin,secondbin);
            print('-djpeg',filename);
            set(gca,'XScale','log');
            filename = sprintf('%s_2_DoubleBin_%s_%s_vs_%s_&_%s_logz_logx',analysis.inputfilename,measure,statistic,firstbin,secondbin);
            print('-djpeg',filename);
                    ...
                    for k = 1:length(fittypes)
                        ...
                        for measurecolumn = 1:size(measuredata,2)
                            ...
                            if (sum(~isnan(y))-length(y(y==0))) > 2
                                ...
                                eval(['parameterfits.' measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.parametervalues(' num2str(fitcount) ',:) = parameter;']);
						...
						filename = sprintf('%s_DoubleBin_%s_%s_vs_%s_%s_%s',analysis.inputfilename,measure,statistic,firstbin,fitfilename,fittypefilename);
                        print('-djpeg',filename);
						set(gca,'ZScale','log');
						filename = sprintf('%s_DoubleBin_%s_%s_vs_%s_%s_%s_logz',analysis.inputfilename,measure,statistic,firstbin,fitfilename,fittypefilename);
						print('-djpeg',filename);
						set(gca,'XScale','log');
						filename = sprintf('%s_DoubleBin_%s_%s_vs_%s_%s_%s_logz_logx',analysis.inputfilename,measure,statistic,firstbin,fitfilename,fittypefilename);
						print('-djpeg',filename);
                        ...
                        if strcmp(currentoption.secondbinfit.fit,'y')
                            ...
                            for m = 1:length(parameter)
                                ...
                                eval(['y = [parameterfits.' measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.' secondbin '.parametervalues(:,' num2str(m) ')];']);
                                ...
                                for n = 1:length(secondfits)
                                    ...
                                    title(char(' ',[measurelabel ' ' statisticlabel ' vs ' firstbintitle ' -- ' fitlabel ' ' fittypelabel],[parametertitle ' vs ' secondbintitle ' -- ' secondfitlabel ' Fits'],' '),'FontSize',12,'FontWeight','b');
                                    set(gca,'FontSize',6);
                                    hold on;
                                    ...
                                    for o = 1:length(secondfittypes)
                                        ...
                                        eval(['parameterfits.' measure '.' statistic '.doublebin.' firstbin '.' fit '.' fittype '.parameter(' num2str(m) ').' secondfit '.' secondfittype '.parametervalues = parameter;']);
                                    ...
                                    clear legendtext;
                                    filename = sprintf('%s_%s_%s_vs_%s_%s_%s_P%d_%s',analysis.inputfilename,measure,statistic,firstbin,fitfilename,fittypefilename,m,secondfitfilename);
                                    print('-djpeg',filename);
									set(gca,'YScale','log');
									filename = sprintf('%s_%s_%s_vs_%s_%s_%s_P%d_%s_logy',analysis.inputfilename,measure,statistic,firstbin,fitfilename,fittypefilename,m,secondfitfilename);
									print('-djpeg',filename);
									set(gca,'XScale','log');
									filename = sprintf('%s_%s_%s_vs_%s_%s_%s_P%d_%s_logy_logx',analysis.inputfilename,measure,statistic,firstbin,fitfilename,fittypefilename,m,secondfitfilename);
									print('-djpeg',filename);
									if strcmp(mnfitteroption.figuresopen,'n'); close all; end;
			...
			if strcmp(currentoption.doublebins.fit,'y')
				...
				eval(['doublefit.parameter(1).parameter = parameterfits.' measure '.' statistic '.doublebin.' firstbin '.' currentoption.doublebins.firstbin.fit '.' currentoption.doublebins.firstbin.fittype '.parameter(1).' currentoption.doublebins.parameter(1).fit '.' currentoption.doublebins.parameter(1).fittype '.parametervalues;']);
				doublefit.parameter(2).fit = currentoption.doublebins.parameter(2).fit;
				eval(['doublefit.parameter(2).parameter = parameterfits.' measure '.' statistic '.doublebin.' firstbin '.' currentoption.doublebins.firstbin.fit '.' currentoption.doublebins.firstbin.fittype '.parameter(2).' currentoption.doublebins.parameter(2).fit '.' currentoption.doublebins.parameter(2).fittype '.parametervalues;']);
				if strcmp(currentoption.doublebins.parameter(3).fit,'none')~=1
					...
					eval(['doublefit.parameter(3).parameter = parameterfits.' measure '.' statistic '.doublebin.' firstbin '.' currentoption.doublebins.firstbin.fit '.' currentoption.doublebins.firstbin.fittype '.parameter(3).' currentoption.doublebins.parameter(3).fit '.' currentoption.doublebins.parameter(3).fittype '.parametervalues;']);
				end
				if strcmp(currentoption.doublebins.parameter(4).fit,'none')~=1
					...
					eval(['doublefit.parameter(4).parameter = parameterfits.' measure '.' statistic '.doublebin.' firstbin '.' currentoption.doublebins.firstbin.fit '.' currentoption.doublebins.firstbin.fittype '.parameter(4).' currentoption.doublebins.parameter(4).fit '.' currentoption.doublebins.parameter(4).fittype '.parametervalues;']);
				end
				...
				ylabel(secondbinlabel,'FontSize',8,'FontWeight','b');
				zlabel([measurelabel ' ' statisticlabel],'FontSize',8,'FontWeight','b');
				title(char(' ',[measurelabel ' ' statisticlabel  ' vs ' firstbintitle ' & ' secondbintitle ' -- Initial Fit'],' '),'FontSize',12,'FontWeight','b');
				set(gca,'FontSize',6);
				...
				filename = sprintf('%s_%s_%s_vs_%s_%s_%s_%s_%s_1st',analysis.inputfilename,measure,statistic,firstbin,filename1,filename2,filename3,filename4,filename5,filename6);
				saveas(gcf,filename,'fig');
				print('-djpeg',filename);
				set(gca,'ZScale','log');
				filename = sprintf('%s_%s_%s_vs_%s_%s_%s_%s_%s_1st_lz',analysis.inputfilename,measure,statistic,firstbin,filename1,filename2,filename3,filename4,filename5,filename6);
				print('-djpeg',filename);
				set(gca,'XScale','log');
				filename = sprintf('%s_%s_%s_vs_%s_%s_%s_%s_%s_1st_lz_lx',analysis.inputfilename,measure,statistic,firstbin,filename1,filename2,filename3,filename4,filename5,filename6);
				print('-djpeg',filename);
				...
				ylabel(secondbinlabel,'FontSize',8,'FontWeight','b');
				zlabel([measurelabel ' ' statisticlabel],'FontSize',8,'FontWeight','b');
				title(char(' ',[measurelabel ' ' statisticlabel  ' vs ' firstbintitle ' & ' secondbintitle ' -- Unweighted Fit'],' '),'FontSize',12,'FontWeight','b');
				set(gca,'FontSize',6);
				legendhandle1 = legendhandle1(1);
				legendhandles = [legendhandle1 legendhandle2];
				legendtext1 = char(' ',sprintf('%s Data',analysis.inputfilename),' ');
				...
				legendtext(2) = {legendtextprep};
				legend(legendhandles,legendtext,'Location','NorthEast');
				filename = sprintf('%s_%s_%s_vs_%s_%s_%s_%s_%s_uw',analysis.inputfilename,measure,statistic,firstbin,filename1,filename2,filename3,filename4,filename5,filename6);
				saveas(gcf,filename,'fig');
				print('-djpeg',filename);
				set(gca,'ZScale','log');
				filename = sprintf('%s_%s_%s_vs_%s_%s_%s_%s_%s_uw_lz',analysis.inputfilename,measure,statistic,firstbin,filename1,filename2,filename3,filename4,filename5,filename6);
				print('-djpeg',filename);
				set(gca,'XScale','log');
				filename = sprintf('%s_%s_%s_vs_%s_%s_%s_%s_%s_uw_lz_lx',analysis.inputfilename,measure,statistic,firstbin,filename1,filename2,filename3,filename4,filename5,filename6);
				print('-djpeg',filename);
				...
				ylabel(secondbinlabel,'FontSize',8,'FontWeight','b');
				zlabel([measurelabel ' ' statisticlabel],'FontSize',8,'FontWeight','b');
				title(char(' ',[measurelabel ' ' statisticlabel  ' vs ' firstbintitle ' & ' secondbintitle ' -- Weighted Fit'],' '),'FontSize',12,'FontWeight','b');
				set(gca,'FontSize',6);
				legendhandle1 = legendhandle1(1);
				legendhandles = [legendhandle1 legendhandle2];
				legendtext1 = char(' ',sprintf('%s Data',analysis.inputfilename),' ');
				...
				legendtext(2) = {legendtextprep};
				legend(legendhandles,legendtext,'Location','NorthEast');
				filename = sprintf('%s_%s_%s_vs_%s_%s_%s_%s_%s_w',analysis.inputfilename,measure,statistic,firstbin,filename1,filename2,filename3,filename4,filename5,filename6);
				saveas(gcf,filename,'fig');
				print('-djpeg',filename);
				set(gca,'ZScale','log');
				filename = sprintf('%s_%s_%s_vs_%s_%s_%s_%s_%s_w_lz',analysis.inputfilename,measure,statistic,firstbin,filename1,filename2,filename3,filename4,filename5,filename6);
				print('-djpeg',filename);
				set(gca,'XScale','log');
				filename = sprintf('%s_%s_%s_vs_%s_%s_%s_%s_%s_w_lz_lx',analysis.inputfilename,measure,statistic,firstbin,filename1,filename2,filename3,filename4,filename5,filename6);
				print('-djpeg',filename);
				if strcmp(mnfitteroption.figuresopen,'n'); close all; end;
			end
		end
