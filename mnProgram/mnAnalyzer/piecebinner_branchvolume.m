...
% For every motoneuron in the population, do the following:
	...
	data.individual(i).maxbranchvolume        = max([[mnpop.mn(i).piece(2:end).proximalbranchvolume] [mnpop.mn(i).piece(2:end).distalbranchvolume]]);
...
% Define the population branch volume bin middles
maxbranchvolumebin = 1 + floor(max([data.individual.maxbranchvolume]) / data.branchvolumebinwidth);
for branchvolumebins = 1:maxbranchvolumebin
    data.population.branchvolumebinmiddles(branchvolumebins) = (branchvolumebins * data.branchvolumebinwidth) - (data.branchvolumebinwidth / 2);
end
...
% For every motoneuron in the population, do the following:
    ...
    % For every piece in this motoneuron, do the following:
		...
		currentproximalbranchvolume        = mnpop.mn(i).piece(j).proximalbranchvolume;
        currentdistalbranchvolume          = mnpop.mn(i).piece(j).distalbranchvolume;
        currentproximalbranchvolumebin     = 1 + floor(currentproximalbranchvolume / data.branchvolumebinwidth);
        currentdistalbranchvolumebin       = 1 + floor(currentdistalbranchvolume / data.branchvolumebinwidth);
...
%                     disp(sprintf('      Partial Dist RD: %.2f',data.individual(i).radialdistancebins(radialdistancebin).piece(nextpiece).distalradialdistance)); 
		...
		% Put the piece into the appropriate branch volume bin  
		
        for branchvolumebin = currentproximalbranchvolumebin:currentdistalbranchvolumebin
        % Repeat for as many partial pieces as there will end up being 

            if     isfield(data.individual(i),'branchvolumebins')==0; nextpiece = 1;
            elseif branchvolumebin > length(data.individual(i).branchvolumebins); nextpiece = 1;
            elseif isfield(data.individual(i).branchvolumebins(branchvolumebin),'piece')==0; nextpiece = 1;
            else nextpiece = 1 + length(data.individual(i).branchvolumebins(branchvolumebin).piece);
            end

            temppiece = setfield(mnpop.mn(i).piece(j),'partial','n');
            data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece) = temppiece;
            clear temppiece;

            if currentdistalbranchvolumebin ~= currentproximalbranchvolumebin
            % If the piece lies in two or more branch length bins, split the piece and bin the current partial piece
            
                data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).partial = 'y';
                
                if branchvolumebin == currentproximalbranchvolumebin
                % If the current partial piece falls in the proximal branch length bin
                    
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distaltype           = [];
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).rallratio            = [];
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).rallratio1           = [];
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).rallratio2           = [];
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).rallratio3           = [];
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).daughterratio        = [];
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughterratio  = [];
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughter2ratio = [];
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).volume               = (branchvolumebin*data.branchvolumebinwidth) - mnpop.mn(i).piece(j).proximalbranchvolume;
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length               = lengthsolver(data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).volume, data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter, (  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ));
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter       = mnpop.mn(i).piece(j).proximaldiameter - ((  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ) * data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length);
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea          = pi*((data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)+(data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))*(((((data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)-(data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))^2)+data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length^2)^0.5);
                    
                    [xdirection, ydirection, zdirection]                                               = directionfunction(mnpop.mn(i).piece(j).elevation,mnpop.mn(i).piece(j).azimuth);                   
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition      = mnpop.mn(i).piece(j).proximalxposition + xdirection * data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition      = mnpop.mn(i).piece(j).proximalyposition + ydirection * data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition      = mnpop.mn(i).piece(j).proximalzposition + zdirection * data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalbranchlength   = data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchlength + data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length;
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalbranchvolume   = data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchvolume + data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).volume;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalradialdistance = ( (data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition-mnpop.mn(i).xposition)^2 ...
                                                                                                         + (data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition-mnpop.mn(i).yposition)^2 ...
                                                                                                         + (data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition-mnpop.mn(i).zposition)^2 )^0.5;
                    
                    
                elseif branchvolumebin > currentproximalbranchvolumebin && branchvolumebin < currentdistalbranchvolumebin
                % If the current partial piece falls between the proximal and distal branch length bins
                    
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaltype         = [];
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distaltype           = [];
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).rallratio            = [];
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).rallratio1           = [];
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).rallratio2           = [];
                    ...
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughterratio  = [];
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughter2ratio = [];
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).volume               = data.branchvolumebinwidth;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter     = data.individual(i).branchvolumebins(branchvolumebin-1).piece(length(data.individual(i).branchvolumebins(branchvolumebin-1).piece)).distaldiameter;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length               = lengthsolver(data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).volume, data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter, (  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ));
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter       = data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter - ((  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ) * data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length);
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea          = pi*((data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)+(data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))*(((((data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)-(data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))^2)+data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length^2)^0.5);
                    
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximalxposition      = data.individual(i).branchvolumebins(branchvolumebin-1).piece(end).distalxposition;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximalyposition      = data.individual(i).branchvolumebins(branchvolumebin-1).piece(end).distalyposition;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximalzposition      = data.individual(i).branchvolumebins(branchvolumebin-1).piece(end).distalzposition;
                    [xdirection, ydirection, zdirection]                                                     = directionfunction(mnpop.mn(i).piece(j).elevation,mnpop.mn(i).piece(j).azimuth);                   
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition        = mnpop.mn(i).piece(j).proximalxposition + xdirection * data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition        = mnpop.mn(i).piece(j).proximalxposition + ydirection * data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition        = mnpop.mn(i).piece(j).proximalxposition + zdirection * data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchvolume     = data.individual(i).branchvolumebins(branchvolumebin-1).piece(end).distalbranchvolume;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalbranchvolume       = data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchvolume + data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).volume;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximalradialdistance = ( (data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximalxposition-mnpop.mn(i).xposition)^2 ...
                                                                                                               + (data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximalyposition-mnpop.mn(i).yposition)^2 ...
                                                                                                               + (data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximalzposition-mnpop.mn(i).zposition)^2 )^0.5;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalradialdistance = ( (data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition-mnpop.mn(i).xposition)^2 ...
                                                                                                             + (data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition-mnpop.mn(i).yposition)^2 ...
                                                                                                             + (data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition-mnpop.mn(i).zposition)^2 )^0.5;
                    
                    
                elseif branchvolumebin == currentdistalbranchvolumebin
                % If the current partial piece falls in the distal branch length bin
                    
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaltype     = [];
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).volume           = mnpop.mn(i).piece(j).distalbranchvolume - data.individual(i).branchvolumebins(branchvolumebin-1).piece(end).distalbranchvolume;
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length           = lengthsolver(data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).volume, data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter, (  -(mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ));
					data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter = data.individual(i).branchvolumebins(branchvolumebin-1).piece(length(data.individual(i).branchvolumebins(branchvolumebin-1).piece)).distaldiameter;
                    data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea      = pi*((data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)+(data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))*(((((data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)-(data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))^2)+data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length^2)^0.5);
                    ...
                    %disp('End-piece in branchvolume bin');
					%keyboard;
					
                end
				
				if data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).length < 0 || ...
				   data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter < 0 || ...
				   data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea < 0 || ...
				   data.individual(i).branchvolumebins(branchvolumebin).piece(nextpiece).volume < 0
				   disp('Error in piecebinner (876)');
				   %keyboard;
				end
		...
		% Put the piece into the appropriate branch volume bifurcation bin  
		
        for branchvolumebin = currentproximalbranchvolumebin:currentdistalbranchvolumebin
        % Repeat for as many partial pieces as there will end up being 

            if     isfield(data.individual(i).bifurcation,'branchvolumebins')==0; nextpiece = 1;
            elseif branchvolumebin > length(data.individual(i).bifurcation.branchvolumebins); nextpiece = 1;
            elseif isfield(data.individual(i).bifurcation.branchvolumebins(branchvolumebin),'piece')==0; nextpiece = 1;
            else nextpiece = 1 + length(data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece);
            end

            temppiece = setfield(mnpop.mn(i).piece(j),'partial','n');
            data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece) = temppiece;
            clear temppiece;

            if currentdistalbranchvolumebin ~= currentproximalbranchvolumebin
            % If the piece lies in two or more branch length bins, split the piece and bin the current partial piece
            
                data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).partial = 'y';
                
                if branchvolumebin == currentproximalbranchvolumebin
                % If the current partial piece falls in the proximal branch length bin
                    
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distaltype           = [];
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio            = [];
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio1           = [];
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio2           = [];
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio3           = [];
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).daughterratio        = [];
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughterratio  = [];
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughter2ratio = [];
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).volume               = (branchvolumebin*data.branchvolumebinwidth) - mnpop.mn(i).piece(j).proximalbranchvolume;
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length               = lengthsolver(data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).volume, data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter, (  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ));
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter       = mnpop.mn(i).piece(j).proximaldiameter - ((  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ) * data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length);
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea          = pi*((data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)+(data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))*(((((data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)-(data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))^2)+data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length^2)^0.5);
                    
                    [xdirection, ydirection, zdirection]                                               = directionfunction(mnpop.mn(i).piece(j).elevation,mnpop.mn(i).piece(j).azimuth);                   
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition      = mnpop.mn(i).piece(j).proximalxposition + xdirection * data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition      = mnpop.mn(i).piece(j).proximalyposition + ydirection * data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition      = mnpop.mn(i).piece(j).proximalzposition + zdirection * data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalbranchlength   = data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchlength + data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length;
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalbranchvolume   = data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchvolume + data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).volume;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalradialdistance = ( (data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition-mnpop.mn(i).xposition)^2 ...
                                                                                                         + (data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition-mnpop.mn(i).yposition)^2 ...
                                                                                                         + (data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition-mnpop.mn(i).zposition)^2 )^0.5;
                    
                    
                elseif branchvolumebin > currentproximalbranchvolumebin && branchvolumebin < currentdistalbranchvolumebin
                % If the current partial piece falls between the proximal and distal branch length bins
                    
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaltype         = [];
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distaltype           = [];
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio            = [];
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio1           = [];
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio2           = [];
                    ...
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughterratio  = [];
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughter2ratio = [];
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).volume               = data.branchvolumebinwidth;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter     = data.individual(i).bifurcation.branchvolumebins(branchvolumebin-1).piece(length(data.individual(i).bifurcation.branchvolumebins(branchvolumebin-1).piece)).distaldiameter;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length               = lengthsolver(data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).volume, data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter, (  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ));
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter       = data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter - ((  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ) * data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length);
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea          = pi*((data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)+(data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))*(((((data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)-(data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))^2)+data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length^2)^0.5);
                    
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximalxposition      = data.individual(i).bifurcation.branchvolumebins(branchvolumebin-1).piece(end).distalxposition;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximalyposition      = data.individual(i).bifurcation.branchvolumebins(branchvolumebin-1).piece(end).distalyposition;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximalzposition      = data.individual(i).bifurcation.branchvolumebins(branchvolumebin-1).piece(end).distalzposition;
                    [xdirection, ydirection, zdirection]                                                     = directionfunction(mnpop.mn(i).piece(j).elevation,mnpop.mn(i).piece(j).azimuth);                   
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition        = mnpop.mn(i).piece(j).proximalxposition + xdirection * data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition        = mnpop.mn(i).piece(j).proximalxposition + ydirection * data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition        = mnpop.mn(i).piece(j).proximalxposition + zdirection * data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchvolume     = data.individual(i).bifurcation.branchvolumebins(branchvolumebin-1).piece(end).distalbranchvolume;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalbranchvolume       = data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchvolume + data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).volume;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximalradialdistance = ( (data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximalxposition-mnpop.mn(i).xposition)^2 ...
                                                                                                               + (data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximalyposition-mnpop.mn(i).yposition)^2 ...
                                                                                                               + (data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximalzposition-mnpop.mn(i).zposition)^2 )^0.5;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalradialdistance = ( (data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition-mnpop.mn(i).xposition)^2 ...
                                                                                                             + (data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition-mnpop.mn(i).yposition)^2 ...
                                                                                                             + (data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition-mnpop.mn(i).zposition)^2 )^0.5;
                ...
                % If the current partial piece falls in the distal branch length bin
                    ...
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaltype     = [];
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).volume           = mnpop.mn(i).piece(j).distalbranchvolume - data.individual(i).bifurcation.branchvolumebins(branchvolumebin-1).piece(end).distalbranchvolume;
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length           = lengthsolver(data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).volume, data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter, (  -(mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ));
					data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter = data.individual(i).bifurcation.branchvolumebins(branchvolumebin-1).piece(length(data.individual(i).bifurcation.branchvolumebins(branchvolumebin-1).piece)).distaldiameter;
                    data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea      = pi*((data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)+(data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))*(((((data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)-(data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))^2)+data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length^2)^0.5);
				...
				if data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).length < 0 || ...
				   data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter < 0 || ...
				   data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea < 0 || ...
				   data.individual(i).bifurcation.branchvolumebins(branchvolumebin).piece(nextpiece).volume < 0
				   disp('Error in piecebinner (876)');
				   %keyboard;
				end
		...
		% Put the piece into the appropriate branch volume termination bin  
		
        for branchvolumebin = currentproximalbranchvolumebin:currentdistalbranchvolumebin
        % Repeat for as many partial pieces as there will end up being 

            if     isfield(data.individual(i).termination,'branchvolumebins')==0; nextpiece = 1;
            elseif branchvolumebin > length(data.individual(i).termination.branchvolumebins); nextpiece = 1;
            elseif isfield(data.individual(i).termination.branchvolumebins(branchvolumebin),'piece')==0; nextpiece = 1;
            else nextpiece = 1 + length(data.individual(i).termination.branchvolumebins(branchvolumebin).piece);
            end

            temppiece = setfield(mnpop.mn(i).piece(j),'partial','n');
            data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece) = temppiece;
            clear temppiece;

            if currentdistalbranchvolumebin ~= currentproximalbranchvolumebin
            % If the piece lies in two or more branch length bins, split the piece and bin the current partial piece
            
                data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).partial = 'y';
                
                if branchvolumebin == currentproximalbranchvolumebin
                % If the current partial piece falls in the proximal branch length bin
                    
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distaltype           = [];
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio            = [];
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio1           = [];
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio2           = [];
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio3           = [];
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).daughterratio        = [];
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughterratio  = [];
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughter2ratio = [];
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).volume               = (branchvolumebin*data.branchvolumebinwidth) - mnpop.mn(i).piece(j).proximalbranchvolume;
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length               = lengthsolver(data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).volume, data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter, (  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ));
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter       = mnpop.mn(i).piece(j).proximaldiameter - ((  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ) * data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length);
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea          = pi*((data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)+(data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))*(((((data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)-(data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))^2)+data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length^2)^0.5);
                    
                    [xdirection, ydirection, zdirection]                                               = directionfunction(mnpop.mn(i).piece(j).elevation,mnpop.mn(i).piece(j).azimuth);                   
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition      = mnpop.mn(i).piece(j).proximalxposition + xdirection * data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition      = mnpop.mn(i).piece(j).proximalyposition + ydirection * data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition      = mnpop.mn(i).piece(j).proximalzposition + zdirection * data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalbranchlength   = data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchlength + data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length;
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalbranchvolume   = data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchvolume + data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).volume;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalradialdistance = ( (data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition-mnpop.mn(i).xposition)^2 ...
                                                                                                         + (data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition-mnpop.mn(i).yposition)^2 ...
                                                                                                         + (data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition-mnpop.mn(i).zposition)^2 )^0.5;
                    
                    
                elseif branchvolumebin > currentproximalbranchvolumebin && branchvolumebin < currentdistalbranchvolumebin
                % If the current partial piece falls between the proximal and distal branch length bins
                    
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaltype         = [];
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distaltype           = [];
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio            = [];
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio1           = [];
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).rallratio2           = [];
                    ...
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughterratio  = [];
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).parentdaughter2ratio = [];
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).volume               = data.branchvolumebinwidth;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter     = data.individual(i).termination.branchvolumebins(branchvolumebin-1).piece(length(data.individual(i).termination.branchvolumebins(branchvolumebin-1).piece)).distaldiameter;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length               = lengthsolver(data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).volume, data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter, (  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ));
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter       = data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter - ((  (mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ) * data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length);
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea          = pi*((data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)+(data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))*(((((data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)-(data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))^2)+data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length^2)^0.5);
                    
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximalxposition      = data.individual(i).termination.branchvolumebins(branchvolumebin-1).piece(end).distalxposition;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximalyposition      = data.individual(i).termination.branchvolumebins(branchvolumebin-1).piece(end).distalyposition;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximalzposition      = data.individual(i).termination.branchvolumebins(branchvolumebin-1).piece(end).distalzposition;
                    [xdirection, ydirection, zdirection]                                                     = directionfunction(mnpop.mn(i).piece(j).elevation,mnpop.mn(i).piece(j).azimuth);                   
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition        = mnpop.mn(i).piece(j).proximalxposition + xdirection * data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition        = mnpop.mn(i).piece(j).proximalxposition + ydirection * data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition        = mnpop.mn(i).piece(j).proximalxposition + zdirection * data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchvolume     = data.individual(i).termination.branchvolumebins(branchvolumebin-1).piece(end).distalbranchvolume;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalbranchvolume       = data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximalbranchvolume + data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).volume;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximalradialdistance = ( (data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximalxposition-mnpop.mn(i).xposition)^2 ...
                                                                                                               + (data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximalyposition-mnpop.mn(i).yposition)^2 ...
                                                                                                               + (data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximalzposition-mnpop.mn(i).zposition)^2 )^0.5;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalradialdistance = ( (data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalxposition-mnpop.mn(i).xposition)^2 ...
                                                                                                             + (data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalyposition-mnpop.mn(i).yposition)^2 ...
                                                                                                             + (data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distalzposition-mnpop.mn(i).zposition)^2 )^0.5;
                ...
                % If the current partial piece falls in the distal branch length bin
                    ...
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaltype     = [];
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).volume           = mnpop.mn(i).piece(j).distalbranchvolume - data.individual(i).termination.branchvolumebins(branchvolumebin-1).piece(end).distalbranchvolume;
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length           = lengthsolver(data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).volume, data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter, (  -(mnpop.mn(i).piece(j).proximaldiameter - mnpop.mn(i).piece(j).distaldiameter)/mnpop.mn(i).piece(j).length  ));
					data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter = data.individual(i).termination.branchvolumebins(branchvolumebin-1).piece(length(data.individual(i).termination.branchvolumebins(branchvolumebin-1).piece)).distaldiameter;
                    data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea      = pi*((data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)+(data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))*(((((data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).proximaldiameter/2)-(data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter/2))^2)+data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length^2)^0.5);
				...
				if data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).length < 0 || ...
				   data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).distaldiameter < 0 || ...
				   data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).surfacearea < 0 || ...
				   data.individual(i).termination.branchvolumebins(branchvolumebin).piece(nextpiece).volume < 0
				   disp('Error in piecebinner (876)');
				   %keyboard;
				end
